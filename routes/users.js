...
app.post("/verificar", auth, async (req, res) => {
  try {
    ...
    const [rows] = await db.pool.query(
      "SELECT id_usuario, nombre_usur, correo_usur, tel_usur, acumulado_usur, tipo_usur, estatus_usur, ranking_usur, id_sucursal, nombreSucursal, firebase_token FROM usuarios LEFT JOIN sucursales ON sucursales.idSucursal = usuarios.id_sucursal WHERE id_usuario = ?",
      [userId]
    );
